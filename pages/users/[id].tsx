...
const UserDetail = () => {
    ...
    async function fetcher(url: string): Promise<User | null> {
        return  await axios.get(url).then(res => res.data.data);
    }

    const {data, error} = useSWR<any>(`http://localhost:8084/users/${id}`, fetcher);
    if (error) return <div>failed to get detail</div>;
            ...
            <div className="md:flex no-wrap md:-mx-2 ">
                <div className="w-full md:w-3/12 md:mx-2">
                    <div className="bg-white p-3 border-t-4 border-green-400">
                        ...
                        <h1 className="text-gray-900 font-bold text-xl leading-8 my-1">{data.last_name} {data.first_name}</h1>
                    ...
                    <div className="bg-white p-3 shadow-sm rounded-sm">
                        <div className="grid grid-cols-2">
                            ...
                            <div>
                                ...
                                <ul className="list-inside space-y-2">
                                    {data.todos !== null ? data.todos.map((todo: Todo) =>
                                        <li key={todo.id}>
                                            <div className="text-teal-600">{todo.title}</div>
                                            <div className="text-gray-500 text-xs">{todo.body}</div>
                                        </li>
                                    ) : 'test'}
                                </ul>
                            </div>
